...
**Current state:** Protocol works but has confused naming from Qdrant legacy. Simple SQL queries wrapped unnecessarily.
**Next step:** 2h cleanup (quarantine legacy, delete false abstractions)
...
### Priority 1: Quarantine Legacy + Delete Wrappers (2 hours)

**Issue:** Legacy files active, confused method names wrapping 1-line SQL queries
...
1. **Quarantine legacy (1h)** - Move Qdrant-coupled files to `legacy/v2/`, document what they provided

2. **Delete wrappers (1h)**:
   - Remove `get_siblings/genealogy/temporal` from protocol
   - Delete implementations from both backends
   - Delete discovery stubs from orchestrator
   - Add comment: "Query SQL directly when needed"

**NOT doing:**
- ❌ Rewriting indexer (works, can refactor later)
- ❌ Building discovery processors (wait for usage patterns)
- ❌ Adding Qdrant fake methods (it can't do these honestly)

**Why important:** Remove false abstractions inherited from Qdrant's confused naming.

---

### Priority 2: DELETED (YAGNI)

**Previous plan:** Build discovery processors

**New understanding:** These wrap simple SQL queries. Don't build until:
1. You have 2-3 concrete usage patterns
2. Duplication proves abstraction needed
3. Clear relationship semantics emerge

**When needed, query directly:**
```python
# Same document chunks:
db.execute("SELECT * FROM chunks WHERE file_path = ? AND id != ?", file_path, anchor_id)

# Session chunks:
db.execute("SELECT * FROM chunks WHERE session_id = ?", session_id)

# Temporal window:
db.execute("SELECT * FROM chunks WHERE timestamp BETWEEN ? AND ?", start, end)
```
...
### Priority 3 (Optional): Rewrite Indexer (4 hours)
...
| `get_siblings/genealogy/temporal` | Wrap 1-line queries | False abstraction, confused naming |
| Discovery stubs | Raise NotImplementedError | Incomplete processors for unused feature |

### What's Missing ❌

| Feature | Needed For | Priority |
|---------|-----------|----------|
| Cleanup wrappers | Remove false abstractions | P1 (2h) |
| Integration tests | Regression prevention | P4 (optional) |
| Protocol-based indexer | Backend independence | P3 (optional) |
| HNSW backend | Local vectors (no Docker) | Future |
| Relationship table | Graph features | Future (when needed) |
