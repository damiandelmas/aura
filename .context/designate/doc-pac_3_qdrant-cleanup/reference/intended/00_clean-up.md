...
### RETRIEVE Domain
```
Input:  Query config (search + ranking)
Process:
  1. Build processor chain from config
  2. SearchProcessor → VectorStore.search()
  3. RankingProcessor → sort/score
  4. If graph needed → Query SQL directly (no wrapper)
...
**Dependencies:** Only VectorStore protocol (no discovery wrappers)
...
User: "Find related chunks"
    ↓
Direct SQL query (no wrapper):
    db.execute("SELECT * FROM chunks WHERE file_path = ?", ...)
    OR
    db.execute("SELECT * FROM relationships WHERE source_id = ?", ...)
    ↓
Results: List of chunks/edges
```

**YAGNI approach:** Don't wrap until 2-3 usage patterns prove abstraction needed.
**Explicit queries:** Clear semantics, no vague names like "siblings".

---

## Enables vs Blocks

### What Cleanup ENABLES

**1. Clear Semantics**
```
Instead of:
  get_siblings() - vague, inherited from Qdrant
...
**Cannot do with confused wrappers:**
- ✗ Understand what "siblings" means (spatial? semantic? session?)
- ✗ Know if Qdrant can deliver (it fakes file_path queries with similarity)
- ✗ Query clearly (vague names hide actual semantics)
- ✗ Refactor safely (unclear what depends on wrapper behavior)
- ✗ Build on foundation (false abstractions propagate confusion)
