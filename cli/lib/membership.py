...
changes: join / leave / rename — "membership changed, a new member entered". It is NOT
...
The ambient-pending flag belongs to the ambient layer (the hook reads it); membership
...
def pending_path(target: str) -> Path:
    ...
    This flag is the ambient layer's mechanism — membership only sets it; the hook owns
...
def ambient_pending_root() -> Path:
    return _state_root() / "ambient-pending"


def reap_ambient_pending(ttl_seconds: int = 3600) -> dict[str, Any]:
    """Backstop GC for ambient-pending flags. The hook clears its flag on consume; a
    seat that died before consuming would leave one forever. Drop flags older than ttl."""
    import time

    root = ambient_pending_root()
    if not root.is_dir():
        return {"ok": True, "reaped": 0, "scanned": 0}
    now = time.time()
    reaped = scanned = 0
    for path in root.glob("*.json"):
        ...
        try:
            ...
            continue
    return {"ok": True, "reaped": reaped, "scanned": scanned}
