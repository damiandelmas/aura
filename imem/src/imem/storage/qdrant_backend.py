...
class QdrantVectorStore:
    ...
    def get_siblings(
        ...
        Args:
            ...
            same_section: If True, filter by same section_type

        Returns:
            List of sibling SearchResult objects
        """
        try:
            # Get anchor chunk to find its file_path
            anchor = self.get_by_ids([chunk_id])
            if not anchor:
                return []

            file_path = anchor[0].metadata.get('file_path')
            section_type = anchor[0].metadata.get('section_type')

            # Build filter
            must_conditions = [
                FieldCondition(key='file_path', match=MatchValue(value=file_path))
            ]
            if same_section and section_type:
                must_conditions.append(
            ...
            # Get all chunks from same file/section
            # Use anchor's content as query vector for semantic ordering
            siblings = self.search(
                query=anchor[0].content,
                limit=limit + 1,  # +1 to exclude anchor later
                filters={'file_path': file_path}
            )

            # Remove anchor chunk and limit
            siblings = [s for s in siblings if s.id != chunk_id][:limit]
    ...
    def get_genealogy(
        ...
        Args:
            chunk_id: Anchor chunk ID
            depth: Ignored (returns all session chunks)
            direction: Ignored (returns all session chunks)

        Returns:
            List of chunks from same conversation session
        """
        try:
            # Get anchor chunk to find its session_id
            anchor = self.get_by_ids([chunk_id])
            if not anchor or not anchor[0].metadata.get('session_id'):
                return []

            session_id = anchor[0].metadata['session_id']

            # Search for all chunks with same session_id
            # Note: Qdrant doesn't have built-in pagination, so this gets all
            query_filter = Filter(must=[
                FieldCondition(key='session_id', match=MatchValue(value=session_id))
            ])

            # Use anchor's content for semantic ordering
            genealogy = self.search(
                query=anchor[0].content,
                limit=100,  # Arbitrary large number for session
    ...
    def get_temporal(
        ...
        Note: Qdrant version uses semantic similarity within same phase
              (doesn't implement strict time window filtering)

        Args:
            chunk_id: Anchor chunk ID
            time_window_days: Ignored (uses semantic similarity instead)
            limit: Max results to return

        Returns:
            List of semantically similar chunks from same phase
        """
        try:
            # Get anchor chunk
            anchor = self.get_by_ids([chunk_id])
            ...
            phase = anchor[0].metadata.get('phase')

            # Find semantically similar chunks from same phase
            temporal = self.search(
                query=anchor[0].content,
                limit=limit + 1,  # +1 to exclude anchor later
            ...
            # Remove anchor chunk
            temporal = [t for t in temporal if t.id != chunk_id][:limit]
