...
## Overview
Implemented granular chunking for conversation archaeology - splitting messages into separate vectors for text, thinking, and tools. Previously, searching conversations returned everything mixed together. Now each component (message text, extended thinking, tool usage, code patches) becomes its own H2 section and vector, enabling precise filtering at query time without pollution.
